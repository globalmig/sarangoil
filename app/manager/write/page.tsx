...
const fmtPyeong = (v: number | null) =>
  v == null ? null : v.toLocaleString("ko-KR", { maximumFractionDigits: 1 });
...
export default function PropertyForm() {
  ...
  return (
    <div className="max-w-[1440px] mx-auto p-6 bg-white shadow rounded my-20 md:mt-40">
      ...
      {errorMsg && (
        <div className="mb-4 rounded border border-red-300 bg-red-50 text-red-700 px-4 py-3 text-sm">
          {errorMsg}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 my-10">
          ...
          <label className="flex flex-col gap-1">
            ...
            <input id="floor" name="floor" placeholder="층수" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">방수 및 욕실</span>
            <input id="rooms_bathrooms" name="rooms_bathrooms" placeholder="방2 욕실1" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">사용승인일</span>
            <input id="approval_date" name="approval_date" placeholder="YYYY-MM-DD" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">도로 현황</span>
            <input id="road_info" name="road_info" placeholder="도로 현황" className="border p-2" onChange={handleChange} type="text" />
          </label>
          ...
          {isLease ? (
            <>
              <label className="flex flex-col gap-1">
                ...
                <input id="deposit" name="deposit" placeholder="예: 100000000" className="border p-2" onChange={handleChange} type="text" />
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-sm text-zinc-600">월세(원)</span>
                <input id="monthly_rent" name="monthly_rent" placeholder="예: 3000000" className="border p-2" onChange={handleChange} type="text" />
              </label>
            </>
          ) : (
            <label className="flex flex-col gap-1">
              <span className="text-sm text-zinc-600">매매가(원)</span>
              <input id="price" name="price" placeholder="예: 1500000000" className="border p-2" onChange={handleChange} type="text" />
            </label>
          )}

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">융자금(원)</span>
            <input id="loan" name="loan" placeholder="예: 300000000" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">저장탱크(기수/용량)</span>
            <input id="storage_tank" name="storage_tank" placeholder="예: 3기 / 90kL" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">주유기(대)</span>
            <input id="pump_count" name="pump_count" placeholder="예: 6" className="border p-2" onChange={handleChange} type="text" />
          </label>
          ...
          <label className="flex flex-col gap-1">
            ...
            <input id="pole" name="pole" placeholder="예: 가능/협의/불가" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">판매량(ℓ/월)</span>
            <input id="sales_volume" name="sales_volume" placeholder="예: 300000" className="border p-2" onChange={handleChange} type="text" />
          </label>

          <label className="flex flex-col gap-1">
            <span className="text-sm text-zinc-600">시설권리금(원)</span>
            <input id="facility_premium" name="facility_premium" placeholder="예: 80000000" className="border p-2" onChange={handleChange} type="text" />
          </label>
        </div>

        <label className="flex flex-col gap-1 col-span-full">
          <span className="text-sm text-zinc-600">특징</span>
          <textarea id="features" name="features" placeholder="특이사항, 조건, 메모 등" className="border p-2 h-48" onChange={handleChange} />
        </label>

        <button
          type="submit"
          disabled={loading}
          className="bg-lime-500 disabled:opacity-60 disabled:cursor-not-allowed text-white py-3 w-full my-10 p-2 rounded hover:bg-lime-600"
        >
          {loading ? "등록 중..." : "등록하기"}
        </button>
      </form>
